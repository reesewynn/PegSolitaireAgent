...
compare_type ParallelAStarLockAgent::getGScore(const entry_type& board) {
    std::shared_lock< std::shared_mutex > r(_gaccess);
    return (g.contains(board))? g[board] : INFINITY;
}

compare_type ParallelAStarLockAgent::getFScore(const entry_type& board) {
    std::shared_lock< std::shared_mutex > r(_faccess);
    return (f.contains(board))? f[board] : INFINITY;
}

bool ParallelAStarLockAgent::getIsInOpenSet(const entry_type item) {
    std::shared_lock< std::shared_mutex > r(_iioaccess);
    return isInOpenSet.contains(item);
}

void ParallelAStarLockAgent::writeGScore(const entry_type& board, compare_type write) {
    std::unique_lock< std::shared_mutex > w(_gaccess);
    g[board] = write;

}
void ParallelAStarLockAgent::writeFScore(const entry_type& board, compare_type write) {
    std::unique_lock< std::shared_mutex > w(_faccess);
    g[board] = write;
}
void ParallelAStarLockAgent::writeIsInOpenSet(const entry_type& board, bool isIn) {
...
bool ParallelAStarLockAgent::search() {
    auto cmp = [this](entry_type const& a, entry_type const& b) -> bool {
        return this->getFScore(a) >= this->getFScore(b);
    };
    priority_queue<entry_type, vector<entry_type>, decltype(cmp)> openSet(cmp);
    ...
    while (!openSet.empty()) {
        ...
        #pragma omp parallel for shared(openSet, isInOpenSet, g, f, pq_lock, _gaccess, _faccess, _iioaccess)
